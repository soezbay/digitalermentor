<template>
    <ion-page>
        <HeaderComponent :title="texts.titel.modulbewertung" :hasInfo="true" />
        <ion-content>
...
export default {
	...
	methods: {
		...
		async submitReview() {
			try {
				...
				await axios.post(`${this.Adress}/bewertung`, bewertungData);
	...
	mounted() {
        this.selectedModul = this.$store.getters.getSelectedModule;
        console.log("SELECTED MODULE", this.selectedModul);
		this.getData();
	},
};
</script>
