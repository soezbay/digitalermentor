	...
	<ion-page>
		<ion-header>
			<ion-toolbar>
				...
				<ion-title>{{ texts.titel.moduluebersicht }}</ion-title>
			...
			<ion-toolbar>
				<ion-item lines="none">
					<ion-select
						...
						:label="texts.studium.studiengang"
						:placeholder="texts.moduluebersicht.studiengangAuswaehlen">
						<ion-select-option
				...
				<ion-item lines="none">
					<ion-toggle v-model="showAsList" @click="onToggleChange()">{{
						texts.moduluebersicht.listenansicht
					}}</ion-toggle>
				</ion-item>
			</ion-toolbar>
		</ion-header>
				...
				<ion-row
					...
					<ion-row class="semesterBlock">
						...
						<template v-if="istEinWahlpflichtmodulImSemester(semester)">
							<ion-col
								class="modulBlock"
								v-for="index in maxWahlpflichtCols(semester)"
								:key="index">
...
export default {
	...
	methods: {
		...
		async fetchPflichtModule() {
			try {
				const response = await fetch(
					`http://localhost:8000/studiengang/pflicht/${this.selectedStudiengang}`
				)
		...
		async fetchWahplfichtModule(selectedStudiengang) {
			try {
				const response = await axios.get(
					`http://localhost:8000/studiengang/wahlpflicht/${this.selectedStudiengang}`
				)
		...
		istEinWahlpflichtmodulImSemester(semester) {
			const filteredModules = this.modules.wahlpflicht.filter(
		...
		AnzahlPflichtmoduleImSemester(semester) {
			const filteredModules = this.modules.pflicht.filter(
				module => module.Semester === semester
			)
